...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformLevelVelocityCommandCfg(
        ...
        debug_vis=True,
        # 单向训练：仅前向速度，ranges 为当前采样范围（课程会从 limit_ranges 内扩展）
        ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(0.0, 0.2), lin_vel_y=(0.0, 0.0), ang_vel_z=(-1, 1)
        ),
...
@configclass
class RewardsCfg:
    """Reward terms for the MDP. Tuned for fast velocity tracking (see REWARDS_DESIGN.md).

    vs Go1 Agile: Go1 uses pose_command (goal point) + distance_to_goal/agile/stall; Go2 uses
    base_velocity + track_lin_vel_xy/track_ang_vel_z. We increase tracking and feet_air_time,
    and relax joint torques/acc/energy so the robot can move faster.
    """

    # -- task (higher weight = stronger drive to match/surpass commanded velocity)
    track_lin_vel_xy = RewTerm(
        func=mdp.track_lin_vel_xy_exp, weight=2.2, params={"command_name": "base_velocity", "std": math.sqrt(0.25)}
    )
    track_ang_vel_z = RewTerm(
        func=mdp.track_ang_vel_z_exp, weight=1.0, params={"command_name": "base_velocity", "std": math.sqrt(0.25)}
    )

    # -- base (keep stability; relax dynamics slightly for fast motion)
    base_linear_velocity = RewTerm(func=mdp.lin_vel_z_l2, weight=-2.0)
    base_angular_velocity = RewTerm(func=mdp.ang_vel_xy_l2, weight=-0.05)
    joint_vel = RewTerm(func=mdp.joint_vel_l2, weight=-0.0005)
    joint_acc = RewTerm(func=mdp.joint_acc_l2, weight=-1.2e-7)
    ...
    # -- feet (higher feet_air_time = encourage running gait with clear flight phase)
